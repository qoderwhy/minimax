...
> **公共列例外**（与代码一致）：关联表 `sys_user_role` / `sys_user_post` / `sys_role_menu` / `sys_role_dept` 仅含业务外键，无 5 公共列；`sys_login_log` 无公共列；`sys_oper_log` 仅有 `create_by / create_time`；`sys_config` 无 `del_flag`（配置表物理删除）。
...
**索引**：`uk_code (code, del_flag)`、`idx_dept_id (dept_id)`
...
**约定**：
1. 取值走 `ConfigService` 类型安全接口：`getValue(key[, default])` / `getInt(key, default)` / `getBoolean(key, default)`
...
| 系统管理 | 角色管理 (`system:role:page` + `system:role:list` + `system:role:detail`) | `system:role:create` / `system:role:update` / `system:role:delete` / `system:role:assign-menu` / `system:role:assign-dept` |
...
-- 此处 hash 由 `BCrypt.hashpw('admin123', BCrypt.gensalt(10))` 生成。
...
> 完整 DDL 在 `qkit-admin/src/main/resources/db/migration/V1.0.0__init.sql`，种子数据在 `V1.0.1__seed.sql`（**以该文件为权威事实源**，本文模板与其保持一致）。
...
3. 写 Flyway `V<major>.<minor>.<patch>__<comment>.sql`（如 `V1.1.0__add_user_avatar.sql`）
